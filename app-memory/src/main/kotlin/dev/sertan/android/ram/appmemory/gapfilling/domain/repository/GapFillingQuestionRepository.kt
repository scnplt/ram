/*
 * RAM (c) by Sertan Canpolat
 *
 * RAM is licensed under a Creative Commons Attribution-NonCommercial 4.0 International License.
 *
 * You should have received a copy of the license along with this work.
 * If not, see <http://creativecommons.org/licenses/by-nc/4.0/>.
 */

package dev.sertan.android.ram.appmemory.gapfilling.domain.repository

import dev.sertan.android.ram.appmemory.gapfilling.domain.model.GapFillingQuestionDto

internal interface GapFillingQuestionRepository {

    suspend fun getQuestions(update: Boolean = false): Result<List<GapFillingQuestionDto>>

    suspend fun saveMatchingQuestionToLocal(question: GapFillingQuestionDto): Boolean

    suspend fun refreshQuestions(): Boolean
}
